...
async function resolveAppointmentIdFromFlowOrder(commerceOrder: string): Promise<string> {
  ...
  // If it's a standard appointment ID (not starting with FLW_), return it directly
  if (!commerceOrder.startsWith("FLW_")) {
  ...
  try {
    ...
    const queryBody = {
      structuredQuery: {
        ...
        where: {
          fieldFilter: {
            field: { fieldPath: "flowCommerceOrder" },
            op: "EQUAL",
            value: { stringValue: commerceOrder }
          }
        },
        limit: 1
      }
    ...
    console.log(`[Flow Resolution] Querying Firestore for appointment with flowCommerceOrder = ${commerceOrder}...`);
    ...
    if (resp.ok) {
      ...
      if (Array.isArray(results) && results.length > 0 && results[0]?.document?.name) {
        const docName = results[0].document.name;
        const resolvedId = docName.split("/").pop();
        console.log(`[Flow Resolution] Successfully resolved ${commerceOrder} to appointment ${resolvedId}`);
        return resolvedId || commerceOrder;
      }
    } else {
      console.error("[Flow Resolution] Firestore runQuery failed:", await resp.text());
    }
  } catch (err) {
    console.error("[Flow Resolution] Error resolving flowCommerceOrder to appointmentId:", err);
  }
  
  return commerceOrder; // fallback to original commerceOrder if resolution fails
}
...
5. Asegúrate de configurar las variables "FLOW_API_KEY" y "FLOW_SECRET_KEY" asociándolas correspondientemente a tus secretos reales con la etiqueta "latest" seleccionada. ¡No olvides guardar los cambios y desplegar para que el contenedor adquiera sus nuevos parámetros!`
  ...
  try {
    ...
    // Generate maximum 45-character long commerceOrder for Flow (prevents errors when appointmentId is a long composite ID)
    const shortOrderId = `FLW_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;

    // Store shortOrderId in Firestore document matching appointmentId so we can resolve it in the webhook return / confirm
    try {
      const projectId = FIRESTORE_PROJECT_ID_RESOLVED;
      const databaseId = FIRESTORE_DATABASE_ID_RESOLVED;
      const apiKey = FIRESTORE_API_KEY_RESOLVED;
      const updateUrl = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/${databaseId}/documents/appointments/${appointmentId}?key=${apiKey}&updateMask.fieldPaths=flowCommerceOrder`;
      await fetch(updateUrl, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          fields: {
            flowCommerceOrder: { stringValue: shortOrderId }
          }
        })
      });
      console.log(`[Flow] Stored flowCommerceOrder ${shortOrderId} for appointment ${appointmentId}`);
    } catch (err) {
      console.warn("[Flow] Could not patch flowCommerceOrder inside Firestore, continuing payment creation anyway", err);
    }
